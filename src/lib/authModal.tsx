...
function LoginForm({ t, refresh, close, toRegister }: { t: (s: string) => string; refresh: () => Promise<any>; close: () => void; toRegister: () => void }) {
    ...
    <>
      ...
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ textAlign: "right", marginTop: -6 }}>
          <Link href="/forgot-password" onClick={close} style={sx("font:500 12px Roboto;color:#8A8F98;text-decoration:underline;")}>{t("Нууц үг мартсан?")}</Link>
        </div>
