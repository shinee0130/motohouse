...
import { PHOTOGRAPHERS, PHOTO_SERVICES } from "@/lib/db/data";
...
export default function PhotoPage() {
  ...
  const chosen = PHOTOGRAPHERS.find((p) => p.id === photographer);

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!user) { authModal.open("login"); return; }
    if (!photographer) return setError(t("Зурагчнаа сонгоно уу."));
    ...
    try {
      await createPhotoBooking({
        photographer: chosen ? chosen.name : photographer,
    ...
    <div style={sx("max-width:1280px;margin:0 auto;padding:clamp(32px,5vw,56px) clamp(20px,4vw,40px);")}>
      <div style={{ animation: "mhfade .5s both" }}>
        <h1 style={sx("font:800 clamp(30px,5vw,46px) Montserrat;color:#fff;margin-top:6px;text-transform:uppercase;")}>
          {t("Зураг авалт захиалга")}
        </h1>
        <p style={sx("font:400 14px Roboto;color:#8A8F98;margin-top:8px;max-width:600px;")}>
          {t("Мотоциклын зураг, reel болон видео бичлэгийн мэргэжлийн багтай холбогдож, тохирох цагаа захиална.")}
        </p>
        ...
        {sent ? (
          <div style={sx("background:#111113;border:1px solid #262626;border-radius:18px;padding:clamp(28px,5vw,48px);margin-top:28px;text-align:center;max-width:560px;")}>
            ...
            <div style={sx("font:400 14px Roboto;color:#A3A3A3;margin-top:10px;")}>
              <b style={{ color: "#fff" }}>{chosen ? loc(chosen.name, chosen.nameEn) : ""}</b> — {t(serviceType)}<br />
              {date} {time}<br />
              {t("Бид удахгүй холбогдож баталгаажуулна.")}
            </div>
            <button
              onClick={() => { setSent(false); setPhotographer(""); setServiceType(""); setDate(""); setTime(""); setModel(""); setNote(""); }}
          ...
          <form onSubmit={submit} style={{ marginTop: 28, display: "flex", flexDirection: "column", gap: 26 }}>
            {/* 1. Зурагчин сонгох */}
            <div>
              <label style={sx(LABEL)}>{t("1. Зурагчин сонгох")}</label>
              <div style={sx("display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:12px;")}>
                {PHOTOGRAPHERS.map((p) => {
                  const on = photographer === p.id;
                  return (
                    <div key={p.id} onClick={() => setPhotographer(p.id)}
                      style={sx(`cursor:pointer;border-radius:14px;padding:16px;background:${on ? "rgba(225,6,19,.08)" : "#111113"};border:1px solid ${on ? "#E10613" : "#262626"};transition:border-color .2s;`)}>
                      <div style={sx("display:flex;align-items:center;gap:12px;")}>
                        <div style={sx(`width:46px;height:46px;border-radius:12px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font:800 16px Montserrat;color:#fff;background:${on ? "#E10613" : "#1c1c1f"};`)}>
                          {loc(p.name, p.nameEn).replace(/\D+/g, "") || "📸"}
                        </div>
                        <div>
                          <div style={sx("font:700 15px Montserrat;color:#fff;")}>{loc(p.name, p.nameEn)}</div>
                      ...
                      <div style={sx("font:400 12px/1.5 Roboto;color:#8A8F98;margin-top:11px;")}>{loc(p.specialty, p.specialtyEn)}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div>
              <label style={sx(LABEL)}>{t("2. Үйлчилгээний төрөл")}</label>
              <div style={sx("display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:8px;")}>
            ...
            <div style={sx("display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:28px;align-items:start;")}>
              {/* 3. Огноо */}
              <div>
                <label style={sx(LABEL)}>{t("3. Огноо")}</label>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
                <div>
                  <label style={sx(LABEL)}>{t("4. Цаг")}</label>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div><label style={sx(LABEL)}>{t("5. Холбоо барих")}</label>
