...
export default function AccountOverview() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      <div style={sx(CARD)}>
        <div style={sx("font:800 22px Montserrat;color:#fff;")}>Сайн уу, {user?.firstName || user?.name || "Райдер"} 👋</div>
        <div style={sx("font:400 14px Roboto;color:#8A8F98;margin-top:6px;")}>
          Тавтай морил. Энд захиалга, хадгалсан бараа, профайлаа удирдана.
        </div>
      </div>
      ...
      <div style={sx(CARD)}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {loaded && recent.length === 0 && <div style={sx("font:400 13px Roboto;color:#8A8F98;")}>Захиалга алга. Дэлгүүрээс захиалга өгөөрэй.</div>}
          {!loaded && <div style={sx("font:400 13px Roboto;color:#8A8F98;")}>Ачаалж байна…</div>}
        </div>
      </div>
    </div>
  );
