...
export default function ProfilePage() {
  ...
  async function changePw(e: React.FormEvent) {
    e.preventDefault();
    setPwErr(""); setPwMsg("");
    if (pw.length < 6) return setPwErr("Шинэ нууц үг дор хаяж 6 тэмдэгт.");
    if (pw !== pw2) return setPwErr("Нууц үг таарахгүй байна.");
    setPwBusy(true);
    try {
      const { error } = await supabase.auth.updateUser({ password: pw });
      if (error) { setPwErr(error.message); return; }
      setPw(""); setPw2(""); setPwMsg("✓ Нууц үг шинэчлэгдлээ.");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={sx(CARD)}>
        ...
        <form onSubmit={save} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <div style={sx("font:400 11px Roboto;color:#8A8F98;margin-top:6px;")}>И-мэйл нь нэвтрэх ID тул өөрчлөгдөхгүй.</div>
          </div>
          {err && <div style={sx("font:500 13px Roboto;color:#ef4444;")}>{err}</div>}
          <button type="submit" style={sx(BTN)}>Хадгалах</button>
      ...
      {/* Нууц үг солих */}
      <div style={sx(CARD)}>
        <div style={sx("font:700 18px Montserrat;color:#fff;margin-bottom:18px;")}>Нууц үг солих</div>
        <form onSubmit={changePw} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <button type="submit" disabled={pwBusy} style={sx(BTN + (pwBusy ? "opacity:.6;" : ""))}>{pwBusy ? "Хадгалж байна…" : "Нууц үг шинэчлэх"}</button>
