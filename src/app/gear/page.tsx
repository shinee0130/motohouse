...
export default function GearPage() {
  ...
  const cats = useMemo(() => ["All", ...Array.from(new Set(GEAR.map((g) => g.category)))], []);
    ...
    <div style={sx("max-width:1280px;margin:0 auto;padding:clamp(32px,5vw,56px) clamp(20px,4vw,40px);")}>
      <div style={{ animation: "mhfade .5s both" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 22 }}>
          {cats.map((c) => (
            <span key={c} onClick={() => setCat(c)} style={sx(chip(cat === c))}>
              {c}
            </span>
          ))}
        </div>
        ...
        <div
          style={sx(
            "display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:20px;margin-top:22px;",
          )}
        >
          {list.map((g) => {
            ...
            return (
              <div
                key={g.id}
                className="mh-card"
                style={sx("background:#111113;border:1px solid #262626;border-radius:14px;overflow:hidden;")}
              >
                <div style={{ position: "relative", height: 200, background: "#fff" }}>
                  <Slot
                    label="Бүтээгдэхүүн зураг"
                    light
                    style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}
                  />
                  <span
                    style={sx(
                      "position:absolute;top:10px;left:10px;z-index:2;font:800 11px Montserrat;letter-spacing:.04em;color:#fff;background:#E10613;padding:5px 9px;border-radius:4px;",
                    )}
                  >
                    SALE -{sale}%
                  </span>
                ...
                <div style={{ padding: "14px 16px" }}>
                  <span style={sx("font:700 13px Montserrat;letter-spacing:.08em;color:#E10613;")}>
                    {"★★★★★".slice(0, g.rating)}
                  </span>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 9, marginTop: 11 }}>
                    <span style={sx("font:400 13px Roboto;color:#8A8F98;text-decoration:line-through;")}>
                      {fmt(g.oldPrice)}
                    </span>
