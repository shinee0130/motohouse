...
import { MOTOS, type MotoStatus } from "@/lib/data";
...
export default function MotorcyclesPage() {
  ...
  return (
    <div
      style={sx(
        "max-width:1280px;margin:0 auto;padding:clamp(32px,5vw,56px) clamp(20px,4vw,40px);",
      )}
    >
      <div style={{ animation: "mhfade .5s both" }}>
        ...
        <div
          style={sx(
            "display:flex;flex-wrap:wrap;gap:14px;align-items:center;margin-top:26px;justify-content:space-between;",
          )}
        >
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {BRANDS.map((b) => (
              <span key={b} onClick={() => setBrand(b)} style={sx(chip(brand === b))}>
                {b}
              </span>
            ))}
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <input
              ...
              className="mh-input"
              style={sx(
                "background:#111113;border:1px solid #262626;border-radius:9px;padding:11px 15px;color:#fff;font:400 14px Roboto;width:180px;outline:none;",
              )}
            />
            <select
              value={sort}
              ...
              style={sx(
                "background:#111113;border:1px solid #262626;border-radius:9px;padding:11px 14px;color:#fff;font:500 13px Roboto;outline:none;cursor:pointer;",
              )}
            >
        ...
        <div
          style={sx(
            "display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:20px;margin-top:14px;",
          )}
        >
          {list.map((m) => (
