    ...
    >
      {done ? (
        <div style={sx("text-align:center;padding:14px 0;")}>
          <div style={sx("font:700 18px Montserrat;color:#22c55e;")}>✓ {t("Нууц үг шинэчлэгдлээ")}</div>
          <div style={sx("font:400 14px Roboto;color:#A3A3A3;margin-top:8px;")}>{t("Таныг оруулж байна…")}</div>
        </div>
      ) : !ready ? (
        <div style={sx("font:500 14px Roboto;color:#8A8F98;text-align:center;padding:14px 0;")}>{t("Уншиж байна…")}</div>
      ) : !hasSession ? (
        <div style={sx("font:400 14px/1.7 Roboto;color:#A3A3A3;text-align:center;padding:14px 0;")}>
          {t("Сэргээх линк хүчингүй эсвэл хугацаа дууссан байна.")} <br />
          <Link href="/forgot-password" style={{ color: "#E10613", fontWeight: 700 }}>{t("Дахин линк авах")}</Link>
        </div>
      ) : (
